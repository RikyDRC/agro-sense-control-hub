...
import { CreditCard, AlertCircle, Settings, CheckCircle } from 'lucide-react';
...
const PaymentGatewaysPage: React.FC = () => {
  ...
  const fetchPaymentConfigs = async () => {
    if (!isRoleSuperAdmin()) {
      toast.error('Only super admins can manage payment gateways');
      return;
    }
  ...
  return (
    <DashboardLayout>
      <div className="mb-6">
        ...
        <p className="text-muted-foreground">Configure payment gateway integration settings</p>
      </div>
      ...
      {!isRoleSuperAdmin() && (
        <Alert variant="destructive" className="mb-6">
          ...
          <AlertDescription>
            Only Super Admins can configure payment gateways.
          </AlertDescription>
        </Alert>
      )}
      ...
      <Card>
        <CardHeader>
          <div className="flex justify-between items-center">
            ...
            <Button variant="outline" onClick={fetchPaymentConfigs} disabled={loading || !isRoleSuperAdmin()}>
              Refresh
            </Button>
          </div>
        </CardHeader>
                ...
                <TabsContent value="stripe">
                  <Form {...stripeForm}>
                    <form onSubmit={stripeForm.handleSubmit(saveStripeConfig)} className="space-y-6">
                      ...
                      <Button 
                        type="submit" 
                        disabled={saving || !isRoleSuperAdmin()}
                        className="w-full"
                      >
